...
from QueuePriorite import QueuePriorite
...
class GrapheXY(Graphe):
    ...
    def init(self, fichierImport=None):
        ...
        try:
            ...
            self.__chemins = pickle.load(open(fichierChemins))
    ...
    def dump(self):
        ...
        if self.__nbCheminsLoad < len(self.__chemins):
            ...
            pickle.dump(self.__chemins, open(fichierChemins, 'wb'), True)
            self.__nbCheminsLoad = len(self.__chemins)
            print('Dump OK')
    ...
    def cheminPlusCourt(self, arcDepart, arcFin):
        ...
        def CPC(arcDepart, arcFin):
            ...
            if arcDepart[1] == arcFin[0]:
                return Chemin([arcDepart[0], arcDepart[1], arcFin[1]], [0, self.attributsArc(arcDepart[0], arcDepart[1]), self.attributsArc(arcDepart[1], arcFin[1])])
    ...
    def genererGraphe(self, nbSommets, nbArcs, distanceMax, germe=None):
        ...
        if nbSommets < 8:
            raise Exception('Nombre de sommets trop petit: ' + repr(nbSommets))
        if distanceMax <= 0:
            raise Exception('Distance max. invalide: ' + repr(distanceMax))

        # calculer le nb de lignes (et de colonnes)
        nbLignes = int(nbSommets**0.5)
        if nbLignes**2 < nbSommets:
            nbLignes = nbLignes + 1
...
if __name__ == '__main__':
    ...
    try:
        ...
        print('OK')
    try:
        a.insererSommet('a', (15.0, 16))
    except:
        print(erreurTypePoint)
    a.insererSommet('a', p)
    p.setXY(2, -3)
    a.insererSommet('b', p)
    p.setXY(5, 4)
